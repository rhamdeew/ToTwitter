...
class Bitly {
    ...
    private $bitlyKey = 'ebae42c4e2186e6e7b4a01bc101dadbac64ab390';
    private $bitlyLogin = '';
    ...
    {
        $result = array();
        $url = $this->bitly_oauth_api . "shorten?access_token=" . $this->bitlyKey . "&longUrl=" . urlencode($longUrl);
        if ($domain != '') {
            $url .= "&domain=" . $domain;
        }
        if ($x_login != '' && $x_apiKey != '') {
    ...
    public function bitly_v3_lookup($data)
    {
        ...
        $url = $this->bitly_oauth_api . "lookup?url=" . $data . "&access_token=" . $this->bitlyKey;
        $output = json_decode($this->bitly_get_curl($url));
